<template>
    <div>
        ...
        <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
            <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
            <el-upload :action="`${constant.baseApi}/sysModule/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
<script name="SysModule" setup>
    ...
    const submitForm = () => {
        formRef.value?.validate((valid) => {
            if (valid) {
                if (form.value.id) {
                        sysModuleApi.update(form.value).then(res => {
                        showMsg(res)
                    })
                } else {
                        sysModuleApi.add(form.value).then(res => {
                        showMsg(res)
                    })
                }
            }
    ...
    const remove = (row) => {
        ...
        ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的系统模块数据项？').then(() => {
                sysModuleApi.remove({
                id: row?.id,
                ids: ids.value
            }).then(res => {
