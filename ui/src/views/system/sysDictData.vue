<template>
    <div>
        ...
        <el-card shadow="hover">
            ...
            <paginationBar v-show="total > 0" v-model:limit="queryParams.size" v-model:page="queryParams.current" :total="total"
                           @pagination="pageList"/>
        </el-card>
        ...
        <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
            <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
            <el-upload :action="`${constant.baseApi}/sysDictData/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
<script name="SysDictData" setup>
    ...
    const pageList = async () => {
        loading.value = true;
            sysDictDataApi.page(queryParams.value).then(res => {
    ...
    const submitForm = () => {
        formRef.value?.validate((valid) => {
            if (valid) {
                if (form.value.id) {
                        sysDictDataApi.update(form.value).then(res => {
                        showMsg(res)
                    })
                } else {
                        sysDictDataApi.add(form.value).then(res => {
                        showMsg(res)
                    })
                }
            }
    ...
    const remove = (row) => {
        ...
        ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的字典数据数据项？').then(() => {
                sysDictDataApi.remove({
                id: row?.id,
                ids: ids.value
            }).then(res => {
                if (res.data) {
    ...
    const downloadTemplate = () => {
            sysDictDataApi.downloadTemplate()
    }
    /** 导出 */
    const exportData = () => {
            sysDictDataApi.exportData()
    }
    /** 导入结果提示 */
