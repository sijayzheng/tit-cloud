<template>
    <div>
        ...
        <el-dialog v-model="dialog.visible" :title="dialog.title" align-center>
            <el-button icon="Download" plain type="primary" @click="downloadTemplate()">下载模板</el-button>
            <el-upload :action="`${constant.baseApi}/sysDictType/import`" accept=".xls,.xlsx" :on-success="uploadSuccess" drag>
...
<script name="SysDictType" setup>
    ...
    const pageList = async () => {
        loading.value = true;
            sysDictTypeApi.page(queryParams.value).then(res => {
    ...
    const submitForm = () => {
        formRef.value?.validate((valid) => {
            if (valid) {
                if (form.value.id) {
                        sysDictTypeApi.update(form.value).then(res => {
                        showMsg(res)
                    })
                } else {
                        sysDictTypeApi.add(form.value).then(res => {
                        showMsg(res)
                    })
                }
            }
    ...
    const remove = (row) => {
        ...
        ElMessageBox.confirm('是否确认删除编号为"' + _ids + '"的字典类型数据项？').then(() => {
                sysDictTypeApi.remove({
                id: row?.id,
                ids: ids.value
            }).then(res => {
                if (res.data) {
    ...
    const downloadTemplate = () => {
            sysDictTypeApi.downloadTemplate()
    }
    /** 导出 */
    const exportData = () => {
            sysDictTypeApi.exportData()
    }
    /** 导入结果提示 */
