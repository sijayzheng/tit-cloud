...
import cn.sijay.system.mapper.SysNoticeMapper;
...
/**
 * <strong>SysNoticeService</strong>
 * <p>
 * 通知公告服务层实现类
 * </p>
 *
...
public class SysNoticeServiceImpl extends ServiceImpl<SysNoticeMapper, SysNotice> implements ISysNoticeService {
     ...
     */
    @Override
    public IPage<SysNoticeDto> page(SysNoticeDto dto, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(dto, SysNoticeDto.class)).convert(SysNotice::toSysNoticeDto);
    }
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class})
    public boolean update(SysNoticeDto dto) {
        checkUnique(dto);
        exists(dto.getId());
        SysNotice sysNotice = getById(dto.getId());
        BeanUtil.copyProperties(dto, sysNotice);
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysNoticeDto dto : ExcelUtil.read(file, SysNoticeDto.class)) {
            if (add(dto)) {
                successCount++;
            } else {
                failCount++;
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(list(), "通知公告", SysNotice.class, response);
    }
}
