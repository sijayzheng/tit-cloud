...
/**
 * <strong>SysPostController</strong>
 * <p>
 * 岗位信息控制层
 * </p>
 *
...
public class SysPostController extends BaseController {
    ...
    @GetMapping("getById")
    public Res<SysPostDto> getById(@RequestParam("id") Long id) {
        return success(sysPostService.getById(id).toSysPostDto());
    }
    ...
    @GetMapping("listAll")
    public Res<List<SysPostDto>> listAll() {
        return success(sysPostService.list().stream().map(SysPost::toSysPostDto).toList());
    }
    ...
    @ApiResponse(responseCode = "200", description = "岗位信息信息列表，如果没有记录，则返回空列表。")
    @GetMapping("page")
    public PageResult<SysPostDto> page(SysPostDto dto, PageQuery pageQuery) {
        return toPageResult(sysPostService.page(dto, pageQuery));
    }

    /**
     * 添加
     *
     * @param dto 添加的岗位信息信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    ...
    @PostMapping("add")
    public Res<Boolean> add(@RequestBody SysPostDto dto) {
        return toBoolean(sysPostService.add(dto), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param dto 修改的岗位信息信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    ...
    @PostMapping("update")
    public Res<Boolean> update(@RequestBody SysPostDto dto) {
        return toBoolean(sysPostService.update(dto), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param id 要删除的岗位信息id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @OprLog(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "id", description = "要删除的岗位信息id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Res<Boolean> remove(@RequestBody Id<Long> id) {
        return toBoolean(sysPostService.remove(id), OperateType.REMOVE);
    }
     ...
     * 下载岗位信息_模板
     */
    @OprLog(value = "下载岗位信息_模板", operateType = OperateType.EXPORT)
    @Operation(summary = "下载岗位信息_模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("岗位信息_模板", SysPostDto.class, response);
    }
