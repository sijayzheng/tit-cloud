...
import cn.sijay.system.service.ISysNoticeService;
...
public class SysNoticeController extends BaseController {
    ...
    @GetMapping("getById")
    public Res<SysNoticeDto> getById(@RequestParam("id") Long id) {
        return success(sysNoticeService.getById(id).toSysNoticeDto());
    }
    ...
    @GetMapping("listAll")
    public Res<List<SysNoticeDto>> listAll() {
        return success(sysNoticeService.list().stream().map(SysNotice::toSysNoticeDto).toList());
    }
    ...
    @ApiResponse(responseCode = "200", description = "通知公告信息列表，如果没有记录，则返回空列表。")
    @GetMapping("page")
    public PageResult<SysNoticeDto> page(SysNoticeDto dto, PageQuery pageQuery) {
        return toPageResult(sysNoticeService.page(dto, pageQuery));
    }

    /**
     * 添加
     *
     * @param dto 添加的通知公告信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    ...
    @PostMapping("add")
    public Res<Boolean> add(@RequestBody SysNoticeDto dto) {
        return toBoolean(sysNoticeService.add(dto), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param dto 修改的通知公告信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    ...
    @PostMapping("update")
    public Res<Boolean> update(@RequestBody SysNoticeDto dto) {
        return toBoolean(sysNoticeService.update(dto), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param id 要删除的通知公告id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @OprLog(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "id", description = "要删除的通知公告id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Res<Boolean> remove(@RequestBody Id<Long> id) {
        return toBoolean(sysNoticeService.remove(id), OperateType.REMOVE);
    }
    ...
    public void export(HttpServletResponse response) {
        sysNoticeService.export(response);
     ...
     * 下载通知公告_模板
     */
    @OprLog(value = "下载通知公告_模板", operateType = OperateType.EXPORT)
    @Operation(summary = "下载通知公告_模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("通知公告_模板", SysNoticeDto.class, response);
    }
