...
import cn.sijay.system.service.ISysModuleService;
 ...
 * @since 2024-04-28
 */
@Tag(name = "系统模块", description = "/sysModule")
...
public class SysModuleController extends BaseController {
    ...
    @ApiResponse(responseCode = "200", description = "系统模块详细信息")
    @GetMapping("getById")
    public Res<SysModuleDto> getById(@RequestParam("id") Long id) {
        return success(sysModuleService.getById(id).toSysModuleDto());
    }

    /**
     * 查询所有记录
     *
     * @return 系统模块信息列表，如果没有记录，则返回空列表。
     */
    @OprLog(value = "查询所有记录", operateType = OperateType.SELECT)
    @Operation(summary = "查询所有记录")
    @ApiResponse(responseCode = "200", description = "系统模块信息列表，如果没有记录，则返回空列表。")
    @GetMapping("listAll")
    public Res<List<SysModuleDto>> listAll() {
        return success(sysModuleService.list().stream().map(SysModule::toSysModuleDto).toList());
    }
    ...
    @ApiResponse(responseCode = "200", description = "系统模块信息列表，如果没有记录，则返回空列表。")
    @GetMapping("page")
    public PageResult<SysModuleDto> page(SysModuleDto dto, PageQuery pageQuery) {
        return toPageResult(sysModuleService.page(dto, pageQuery));
    }

    /**
     * 添加
     *
     * @param dto 添加的系统模块信息，不能为空。如果为空，则添加失败。
     * @return 是否添加成功
     */
    @OprLog(value = "添加", operateType = OperateType.CREATE)
    @Operation(summary = "添加")
    @Parameter(name = "sysModule", description = "添加的系统模块信息，不能为空。如果为空，则添加失败。")
    @ApiResponse(responseCode = "200", description = "是否添加成功")
    @PostMapping("add")
    public Res<Boolean> add(@RequestBody SysModuleDto dto) {
        return toBoolean(sysModuleService.add(dto), OperateType.CREATE);
    }

    /**
     * 修改
     *
     * @param dto 修改的系统模块信息，不能为空。如果为空，则添加失败。
     * @return 是否修改成功
     */
    @OprLog(value = "修改", operateType = OperateType.MODIFY)
    @Operation(summary = "修改")
    @Parameter(name = "sysModule", description = "修改的系统模块信息，不能为空。如果为空，则添加失败。")
    @ApiResponse(responseCode = "200", description = "是否修改成功")
    @PostMapping("update")
    public Res<Boolean> update(@RequestBody SysModuleDto dto) {
        return toBoolean(sysModuleService.update(dto), OperateType.MODIFY);
    }

    /**
     * 删除
     *
     * @param id 要删除的系统模块id，不能为空。如果为空，则删除失败。
     * @return 是否删除成功
     */
    @OprLog(value = "删除", operateType = OperateType.REMOVE)
    @Operation(summary = "删除")
    @Parameter(name = "id", description = "要删除的系统模块id，不能为空。如果为空，则删除失败。")
    @ApiResponse(responseCode = "200", description = "是否删除成功")
    @PostMapping("remove")
    public Res<Boolean> remove(@RequestBody Id<Long> id) {
        return toBoolean(sysModuleService.remove(id), OperateType.REMOVE);
    }
    ...
    public Res<Void> importData(@RequestPart("file") MultipartFile file) {
        return success(sysModuleService.importData(file));
    }
    ...
    public void export(HttpServletResponse response) {
        sysModuleService.export(response);
     ...
     * 下载系统模块_模板
     */
    @OprLog(value = "下载系统模块_模板", operateType = OperateType.EXPORT)
    @Operation(summary = "下载系统模块_模板")
    @ApiResponse(responseCode = "200", description = "是否下载成功")
    @PostMapping("downloadTemplate")
    public void downloadTemplate(HttpServletResponse response) {
        ExcelUtil.exportTemplate("系统模块_模板", SysModuleDto.class, response);
    }
