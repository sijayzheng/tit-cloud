...
import cn.sijay.system.mapper.SysRoleMapper;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements ISysRoleService {
     ...
     */
    @Override
    public IPage<SysRoleDto> page(SysRoleDto dto, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(dto, SysRoleDto.class)).convert(SysRole::toSysRoleDto);
    }
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysRoleDto dto : ExcelUtil.read(file, SysRoleDto.class)) {
            if (add(dto)) {
                successCount++;
            } else {
                failCount++;
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(list(), "角色信息", SysRole.class, response);
    }
}
