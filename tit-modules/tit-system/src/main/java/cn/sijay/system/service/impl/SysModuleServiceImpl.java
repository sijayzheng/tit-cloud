...
import cn.sijay.system.mapper.SysModuleMapper;
...
/**
 * <strong>SysModuleService</strong>
 * <p>
 * 系统模块服务层实现类
 * </p>
 *
...
public class SysModuleServiceImpl extends ServiceImpl<SysModuleMapper, SysModule> implements ISysModuleService {
     ...
     */
    @Override
    public IPage<SysModuleDto> page(SysModuleDto dto, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(dto, SysModuleDto.class)).convert(SysModule::toSysModuleDto);
    }
    ...
    public boolean add(SysModuleDto dto) {
        checkUnique(dto);
        return save(dto.toSysModule());
    }
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class})
    public boolean update(SysModuleDto dto) {
        checkUnique(dto);
        exists(dto.getId());
        SysModule sysModule = getById(dto.getId());
        BeanUtil.copyProperties(dto, sysModule);
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysModuleDto dto : ExcelUtil.read(file, SysModuleDto.class)) {
            if (add(dto)) {
                successCount++;
            } else {
                failCount++;
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(list(), "系统模块", SysModule.class, response);
    }
}
