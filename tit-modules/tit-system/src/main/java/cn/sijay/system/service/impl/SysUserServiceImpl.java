...
import cn.sijay.system.mapper.SysUserMapper;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
     ...
     */
    @Override
    public IPage<SysUserDto> page(SysUserDto dto, PageQuery pageQuery) {
        return page(pageQuery.build(), buildQueryWrapper(dto, SysUserDto.class)).convert(SysUser::toSysUserDto);
    }
    ...
    public String importData(MultipartFile file) {
        int successCount = 0;
        int failCount = 0;
        for (SysUserDto dto : ExcelUtil.read(file, SysUserDto.class)) {
            if (add(dto)) {
                successCount++;
            } else {
                failCount++;
    ...
    public void export(HttpServletResponse response) {
        ExcelUtil.export(list(), "登录用户", SysUser.class, response);
    }
}
